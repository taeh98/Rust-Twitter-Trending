...
const RANK_SUM_TESTS_OUTPUT_FILES_DIRECTORY: &'static str =
    concatcp!(STATS_OUTPUT_FILES_DIRECTORY, "/wilcoxon_rank_sum") as &'static str;

pub(crate) fn make_wilxocon_rank_sum_tests(
    ...
    ALL_VARIABLE_VALUES
        ...
        .for_each(|variable: Variable| {
            let algorithm_names_variable_values: Vec<(&String, &Vec<f64>)> = algorithm_names
                ...
                .collect();
            run_rank_sum_tests_for_variable(&variable, &algorithm_names_variable_values);
        });
}

fn run_rank_sum_tests_for_variable(
    variable: &Variable,
    ...
    print_rank_sum_test_results(results, variable);
}

fn print_rank_sum_test_results(results: Vec<(&String, &String, f64, f64)>, variable: &Variable) {
